...
void TrackDescriptor::create_feat_extractor(const cv::Ptr <ExtractorBase> &extractor) {
    if (!extractor) {
        throw std::runtime_error("Extractor is null.");
    }
    feat_desc0 = extractor;
    //  feat_desc1 = extractor->clone();

    // check if the extractor is ORB
    if (feat_desc0->type() == "ORB") {
...
void TrackDescriptor::feed_new_camera(const CameraData &message) {
    ...
    if (message.sensor_ids.empty() || message.sensor_ids.size() != message.images.size() ||
        ...
        RESET);
        PRINT_ERROR(RED
        "[ERROR]:   - message.sensor_ids.size() = %zu\n"
        RESET, message.sensor_ids.size());
        PRINT_ERROR(RED
        "[ERROR]:   - message.images.size() = %zu\n"
        RESET, message.images.size());
        PRINT_ERROR(RED
        "[ERROR]:   - message.masks.size() = %zu\n"
    ...
    if (num_images == 1) {
        ...
        PRINT_ERROR(RED
        "[ERROR]: invalid number of images passed %zu, we only support mono or stereo tracking", num_images);
        std::exit(EXIT_FAILURE);
    }
}
...
void TrackDescriptor::feed_monocular(const CameraData &message, size_t msg_id) {
    ...
    for (size_t i = 0; i < good_left.size(); i++) {
        ...
        database->update_feature(good_ids_left.at(i), message.timestamp, cam_id, good_left.at(i).pt.x,
...
void TrackDescriptor::perform_detection_monocular(const cv::Mat &img0, const cv::Mat &mask0,
    ...
    cv::Mat desc0_ext;
    this->feat_desc0->detectAndCompute(img0, pts0_ext, desc0_ext);
    // make sure descriptors are f32 type
    if (desc0_ext.type() != CV_32F) {
        desc0_ext.convertTo(desc0_ext, CV_32F);
    }
    ...
    for (size_t i = 0; i < pts0_ext.size(); i++) {
        ...
        if (x_grid < 0 || x_grid >= size.width || y_grid < 0 || y_grid >= size.height || x < 0 || x >= img0.cols ||
            y < 0 || y >= img0.rows) {
            continue;
        }
...
void TrackDescriptor::perform_detection_stereo(const cv::Mat &img0, const cv::Mat &img1, const cv::Mat &mask0,
    ...
    parallel_for_(cv::Range(0, 2), LambdaBody([&](const cv::Range &range) {
        for (int i = range.start; i < range.end; i++) {
            ...
            (is_left ? feat_desc0 : feat_desc1)->detectAndCompute(is_left ? img0 : img1, is_left ? pts0_ext : pts1_ext,
    ...
    for (size_t i = 0; i < matches.size(); i++) {
        ...
        if (x0_grid < 0 || x0_grid >= size0.width || y0_grid < 0 || y0_grid >= size0.height || x0 < 0 ||
            x0 >= img0.cols || y0 < 0 ||
            y0 >= img0.rows) {
            continue;
        }
        ...
        if (x1_grid < 0 || x1_grid >= size1.width || y1_grid < 0 || y1_grid >= size1.height || x1 < 0 ||
            x1 >= img0.cols || y1 < 0 ||
            y1 >= img0.rows) {
            continue;
        }
...
void TrackDescriptor::robust_match(const std::vector <cv::KeyPoint> &pts0, const std::vector <cv::KeyPoint> &pts1,
    ...
    std::cout << "descs type: 0-" << desc0.type() << " 1-" << desc1.type() << std::endl;
