...
#include "descriptors/ExtractorALIKE.h"
#include "descriptors/ExtractorORB.h"
...
VioManager::VioManager(VioManagerOptions &params_) : thread_init_running(false), thread_init_success(false) {
    ...
    if (params.record_timing_information) {
        ...
        if (boost::filesystem::exists(params.record_timing_filepath)) {
            ...
            PRINT_INFO(YELLOW
            "[STATS]: found old file found, deleted...\n"
            RESET);
        }
    ...
    int init_max_features = std::floor(
            (double) params.init_options.init_max_features / (double) params.state_options.num_cameras);
    if (params.use_klt) {
        ...
        trackFEATS = std::shared_ptr<TrackBase>(new TrackDescriptor(
                state->_cam_intrinsics_cameras, init_max_features, state->_options.max_aruco_features,
                params.use_stereo, params.histogram_method,
                params.fast_threshold, params.grid_x, params.grid_y, params.min_px_dist, params.knn_ratio));

        cv::Ptr <ExtractorBase> extractor;
        if (params.model_path.empty()) {
            PRINT_WARNING(RED
            "[ALIKE]: No model path provided, using ORB!\n"
            RESET);
            extractor = ExtractorORB::createExtractor();
        } else {
            PRINT_WARNING(RED
            "[ALIKE]: Using ALIKE model path: %s\n"
            RESET, params.model_path.c_str());
            extractor = ExtractorALIKE::createExtractor(params.model_path,
                                                        params.use_cuda,
                                                        params.top_k,
                                                        params.scores_th,
                                                        params.n_limit,
        ...
        trackFEATS->create_feat_extractor(extractor);
    }

    // Initialize our aruco tag extractor
    if (params.use_aruco) {
        trackARUCO = std::shared_ptr<TrackBase>(
                new TrackAruco(state->_cam_intrinsics_cameras, state->_options.max_aruco_features,
...
trackSIM->
feed_measurement_simulation(timestamp, camids, feats
);
...
if (
is_initialized_vio &&updaterZUPT
!= nullptr && (!params.zupt_only_at_beginning || !has_moved_since_zupt)) {
...
propagator->
clean_old_imu_measurements(timestamp
...
if (!is_initialized_vio) {
PRINT_ERROR(RED
"[SIM]: your vio system should already be initialized before simulating features!!!\n" RESET);
PRINT_ERROR(RED
"[SIM]: initialize your system first before calling feed_measurement_simulation()!!!!\n" RESET);
...
push_back(camid);
message.images.
push_back(cv::Mat::zeros(cv::Size(width, height), CV_8UC1)
);
message.masks.
push_back(cv::Mat::zeros(cv::Size(width, height), CV_8UC1)
);
...
void VioManager::do_feature_propagate_update(const ov_core::CameraData &message) {
    ...
    if (state->_timestamp > message.timestamp) {
        ...
        "image received out of order, unable to do anything (prop dt = %3f)\n"
        RESET,
    ...
    if (state->_timestamp != message.timestamp) {
        PRINT_WARNING(RED
        "[PROP]: Propagator unable to propagate the state forward in time!\n"
        RESET);
        PRINT_WARNING(RED
        "[PROP]: It has been %.3f since last time we propagated\n"
    ...
    while (it1 != feats_lost.end()) {
        ...
        for (const auto &camuvpair: (*it1)->uvs) {
            if (std::find(message.sensor_ids.begin(), message.sensor_ids.end(), camuvpair.first) !=
    ...
    if (state->_options.max_slam_features > 0 && message.timestamp - startup_time >= params.dt_slam_delay &&
        ...
        int valid_amount = (amount_to_add > (int) feats_maxtracks.size()) ? (int) feats_maxtracks.size()
                                                                          : amount_to_add;
    ...
    for (std::pair<const size_t, std::shared_ptr < Landmark>> &landmark : state->_features_SLAM) {
        ...
        bool current_unique_cam =
                std::find(message.sensor_ids.begin(), message.sensor_ids.end(), landmark.second->_unique_camera_id) !=
                message.sensor_ids.end();
    ...
    RESET, time_track);
    PRINT_DEBUG(BLUE
    "[TIME]: %.4f seconds for propagation\n"
    RESET, time_prop);
    PRINT_DEBUG(BLUE
    "[TIME]: %.4f seconds for MSCKF update (%d feats)\n"
    RESET, time_msckf, (int) featsup_MSCKF.size());
    if (state->_options.max_slam_features > 0) {
        PRINT_DEBUG(BLUE
        "[TIME]: %.4f seconds for SLAM update (%d feats)\n"
    ...
    PRINT_DEBUG(BLUE
    "[TIME]: %.4f seconds for re-tri & marg (%d clones in state)\n"
    ...
    if (params.record_timing_information && of_statistics.is_open()) {
        ...
        // Append to the file
        of_statistics << std::fixed << std::setprecision(15) << timestamp_inI << "," << std::fixed
                      << std::setprecision(5) << time_track << ","
    ...
    if (state->_options.do_calib_camera_intrinsics) {
        for (int i = 0; i < state->_options.num_cameras; i++) {
            ...
            PRINT_INFO("cam%d intrinsics = %.3f,%.3f,%.3f,%.3f | %.3f,%.3f,%.3f,%.3f\n", (int) i, calib->value()(0),
                       calib->value()(1),
                       calib->value()(2), calib->value()(3), calib->value()(4), calib->value()(5), calib->value()(6),
                       calib->value()(7));
        }
    }
