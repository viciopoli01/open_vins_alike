    ...
    struct VioManagerOptions {
        ...
        void print_and_load_noise(const std::shared_ptr <ov_core::YamlParser> &parser = nullptr) {
            ...
            if (parser != nullptr) {
                ...
                parser->parse_external("relative_config_imu", "imu0", "accelerometer_noise_density",
                                       imu_noises.sigma_a);
        ...
        void print_and_load_state(const std::shared_ptr <ov_core::YamlParser> &parser = nullptr) {
            if (parser != nullptr) {
                ...
                for (int i = 0; i < state_options.num_cameras; i++) {
                    ...
                    parser->parse_external("relative_config_imucam", "cam" + std::to_string(i), "distortion_model",
                                           dist_model);

                    // Distortion parameters
                    std::vector<double> cam_calib1 = {1, 1, 0, 0};
                    std::vector<double> cam_calib2 = {0, 0, 0, 0};
                    parser->parse_external("relative_config_imucam", "cam" + std::to_string(i), "intrinsics",
                                           cam_calib1);
                    parser->parse_external("relative_config_imucam", "cam" + std::to_string(i), "distortion_coeffs",
                                           cam_calib2);
                    Eigen::VectorXd cam_calib = Eigen::VectorXd::Zero(8);
                    cam_calib << cam_calib1.at(0), cam_calib1.at(1), cam_calib1.at(2), cam_calib1.at(3), cam_calib2.at(
                            0), cam_calib2.at(1),
                    ...
                    parser->parse_external("relative_config_imucam", "cam" + std::to_string(i), "resolution",
                                           matrix_wh);
                    ...
                    if (dist_model == "equidistant") {
                        camera_intrinsics.insert(
                                {i, std::make_shared<ov_core::CamEqui>(matrix_wh.at(0), matrix_wh.at(1))});
                        camera_intrinsics.at(i)->set_value(cam_calib);
                    } else {
                        camera_intrinsics.insert(
                                {i, std::make_shared<ov_core::CamRadtan>(matrix_wh.at(0), matrix_wh.at(1))});
                        camera_intrinsics.at(i)->set_value(cam_calib);
                    }
                ...
                if (use_mask) {
                    for (int i = 0; i < state_options.num_cameras; i++) {
                        ...
                        if (mask.cols != camera_intrinsics.at(i)->w() || mask.rows != camera_intrinsics.at(i)->h()) {
                            PRINT_ERROR(RED
                            "VioManager(): mask size does not match camera!\n"
                            RESET);
                            PRINT_ERROR(RED
                            "\t- mask%d - %s\n"
                            RESET, i, total_mask_path.c_str());
            ...
            if (state_options.num_cameras != (int) camera_intrinsics.size() ||
                ...
                PRINT_ERROR(RED
                "[SIM]: camera calib size does not match max cameras...\n"
                RESET);
                PRINT_ERROR(RED
                "[SIM]: got %d but expected %d max cameras (camera_intrinsics)\n"
                RESET, (int) camera_intrinsics.size(),
                        state_options.num_cameras);
                PRINT_ERROR(RED
                "[SIM]: got %d but expected %d max cameras (camera_extrinsics)\n"
            ...
            for (int n = 0; n < state_options.num_cameras; n++) {
                std::stringstream ss;
                ss << "cam_" << n << "_fisheye:"
                   << (std::dynamic_pointer_cast<ov_core::CamEqui>(camera_intrinsics.at(n)) != nullptr) << std::endl;
                ss << "cam_" << n << "_wh:" << std::endl << camera_intrinsics.at(n)->w() << " x "
            ...
            std::stringstream ss;
            ss << "imu model:" << ((state_options.imu_model == StateOptions::ImuModel::KALIBR) ? "kalibr" : "rpng")
               << std::endl;
        ...
        void print_and_load_trackers(const std::shared_ptr <ov_core::YamlParser> &parser = nullptr) {
            if (parser != nullptr) {
                ...
                parser->parse_config("model_path", model_path);
                parser->parse_config("use_cuda", use_cuda);
                parser->parse_config("top_k", top_k);
                parser->parse_config("scores_th", scores_th);
                parser->parse_config("n_limit", n_limit);
